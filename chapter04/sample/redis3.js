...
const client = redis.createClient(6397, '127.0.0.1');
client.on('error', (err) => {
...
app.get('/airkorea', async (req, res) => {
    await client.lrange('airItems', 0, -1, async (err, cachedItems) => {
        ...
        if (cachedItems.length) {  //data in cache
            res.send(' 데이터가 캐시에 있습니다. <br>관측지역: ${cachedItems[0]} / 관측시간: ${cachedItems[1]} <br>미세먼지 ${cachedItems[2]} 초미세먼지 ${cachedItems[3]} 입니다.')
